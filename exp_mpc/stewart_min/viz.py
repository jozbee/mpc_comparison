...
def compute_leg_lengths(position, rotation_matrix):
    ...
    for i in range(6):
        ...
        platform_point_world = center.flatten() + position + rotation_matrix @ platform_point_local
...
def visualize_trajectory(trajectory, dt=0.02, sim_rate=1.0, fps=30):
    ...
    gs = gridspec.GridSpec(2, 2, height_ratios=[1.0, 0.1], width_ratios=[1.0, 0.7], wspace=0.35, hspace=0.35)
    ...
    x_lims = np.array([point[0] for point in bots] + [point[0] for point in tops])
    x_min = np.min(x_lims) - 0.5
    x_max = np.max(x_lims) + 0.5
    y_lims = np.array([point[1] for point in bots] + [point[1] for point in tops])
    y_min = np.min(y_lims) - 0.5
    y_max = np.max(y_lims) + 0.5
    z_lims = np.array([point[2] for point in bots] + [point[2] for point in tops])
    ...
    ax_legs.axhline(y=leg_min, color='r', linestyle='-', alpha=0.5, label='Min Length')
    ax_legs.axhline(y=leg_max, color='r', linestyle='-', alpha=0.5, label='Max Length')
    
    # Initialize visualization elements
    base_polygon, = ax_3d.plot([], [], [], 'ko-', linewidth=2, markersize=5)
    platform_polygon, = ax_3d.plot([], [], [], 'bo-', linewidth=2, markersize=5)
    legs = [ax_3d.plot([], [], [], 'g-', linewidth=1)[0] for _ in range(6)]
    leg_bars = ax_legs.bar(range(1, 7), [0] * 6, color='black', alpha=0.7)
    leg_text = ax_legs.text(0.02, 0.95, '', transform=ax_legs.transAxes, verticalalignment='top')
    ...
    base_polygon.set_3d_properties(base_z)
    ...
    t_interp = np.arange(0, dt * num_points, dt * sim_rate)
    ...
    interp_trajectory = list(zip(x_interp, y_interp, z_interp, roll_interp, pitch_interp, yaw_interp))
    ...
    def update(i):
        ...
        tops_world = [center.flatten() + position + R @ (p - center.flatten()) for p in tops]
        ...
        platform_polygon.set_3d_properties(platform_z)
        ...
        for j in range(6):
            ...
            legs[j].set_data([start_point[0], end_point[0]], [start_point[1], end_point[1]])
            legs[j].set_3d_properties([start_point[2], end_point[2]])
        ...
        leg_text.set_text(f"Position: ({x:.2f}, {y:.2f}, {z:.2f})\nOrientation: ({np.degrees(roll):.2f}°, {np.degrees(pitch):.2f}°, {np.degrees(yaw):.2f}°)")
        ...
        return (platform_polygon, *legs, *leg_bars, leg_text, progress_bar[0], time_text)
