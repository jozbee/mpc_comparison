...
def animate_cost_trajectory(
    ...
    """Animate the horizon components of the MPC cost function.
    ...
    * control
    ...
    head_labels = ["id", "scaled", "full"]
    lines_head: list[mpl_lines.Line2D] = []
    for data, label, color in zip(all_head_acc, head_labels, colors(3)):
        lines = ax_head.plot(data[init_index], color=color, label=label)
        lines_head.append(lines[0])
    ax_head.legend(**legend_kwargs)
    ...
    omega_labels = ["id", "scaled", "full"]
    lines_omega: list[mpl_lines.Line2D] = []
    for data, label, color in zip(all_omega, omega_labels, colors(3)):
        lines = ax_omega.plot(data[init_index], color=color, label=label)
        lines_omega.append(lines[0])
    ax_omega.legend(**legend_kwargs)
    ...
    for i, color in enumerate(colors(6)):
        lines = ax_leg_pos.plot(
            leg_pos[init_index, :, i], color=color, label=f"leg {i}"
        )
        lines_leg_pos.append(lines[0])
    ax_leg_pos.legend(**legend_kwargs)
    ...
    for i, color in enumerate(colors(6)):
        lines = ax_leg_vel.plot(
            leg_vel[init_index, :, i], color=color, label=f"leg {i}"
        )
        lines_leg_vel.append(lines[0])
    ax_leg_vel.legend(**legend_kwargs)
    ...
    for i, color in enumerate(colors(12)):
        lines = ax_leg_angle.plot(
            joint_angle[init_index, :, i], color=color, label=f"joint {i}"
        )
        lines_joint_angle.append(lines[0])
    ax_leg_angle.legend(**legend_kwargs)
    ...
    line_yaw = ax_yaw.plot(yaw[init_index], color=colors(1)[0], label="yaw")[0]
    ax_yaw.legend(**legend_kwargs)
    ...
    for i, color in enumerate(colors(6)):
        lines = ax_control.plot(
            control[init_index, :, i], color=color, label=f"control {i}"
        )
        lines_control.append(lines[0])
    ax_control.legend(**legend_kwargs)
    ...
    head_acc_terms = jnp.mean(full_head_acc, axis=-1)
    omega_terms = jnp.mean(full_omega, axis=-1)
    leg_pos_terms = jnp.sum(jnp.mean(leg_pos, axis=1), axis=-1)
    leg_vel_terms = jnp.sum(jnp.mean(leg_vel, axis=1), axis=-1)
    joint_angle_terms = jnp.sum(jnp.mean(joint_angle, axis=1), axis=-1)
    yaw_terms = jnp.mean(yaw, axis=-1)
    control_terms = jnp.sum(jnp.mean(control, axis=1), axis=-1)
    all_terms = [
        head_acc_terms,
        omega_terms,
        leg_pos_terms,
    ...
    bars_perc = ax_perc.bar(
        shorthand_perc_labels,
        all_perc[:, init_index],
        label=perc_labels,
        color=colors(all_perc.shape[0]),
    )
    ax_perc.legend(**legend_kwargs)
    ...
    def update(frame_num):
        ...
        for line, data in zip(lines_head, all_head_acc):
            line.set_data(jnp.arange(data[traj_index].size), data[traj_index])

        for line, data in zip(lines_omega, all_omega):
            line.set_data(jnp.arange(data[traj_index].size), data[traj_index])

        for i, line in enumerate(lines_leg_pos):
            line.set_data(
                jnp.arange(leg_pos[traj_index, :, i].size),
                leg_pos[traj_index, :, i],
            )

        for i, line in enumerate(lines_leg_vel):
            line.set_data(
                jnp.arange(leg_vel[traj_index, :, i].size),
                leg_vel[traj_index, :, i],
            )

        for i, line in enumerate(lines_joint_angle):
            line.set_data(
                jnp.arange(joint_angle[traj_index, :, i].size),
                joint_angle[traj_index, :, i],
            )

        line_yaw.set_data(jnp.arange(yaw[traj_index].size), yaw[traj_index])

        for i, line in enumerate(lines_control):
            line.set_data(
                jnp.arange(control[traj_index, :, i].size),
                control[traj_index, :, i],
            )

        for i, bar in enumerate(bars_perc):
            bar.set_height(all_perc[i, traj_index])

        updates = lines_head + lines_omega + lines_leg_pos + lines_leg_vel
        updates += lines_joint_angle + [line_yaw] + lines_control
        updates += list(bars_perc)
        return updates
