...
def load_sms_data(filepath) -> pd.DataFrame:
    """
...
def visualize_motion_data(df):
    ...
    try:
        ...
        plt.subplot(3, 1, 1)
        plt.plot(
            df[time_col], df["sesmt.md.merged_frame.xyz[0]"], label="X Position"
        )
        ...
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.xyz_vel[0]"],
            label="X Velocity",
        )
        ...
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.xyz_vel[2]"],
            label="Z Velocity",
            color="green",
        )
        ...
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.xyz_acc[0]"],
        ...
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.xyz_acc[2]"],
        ...
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.ang_vel[0]"],
        ...
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.ang_vel[2]"],
        ...
        ax.plot(
            df["sesmt.md.merged_frame.xyz[0]"],
        ...
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.gravity[0]"],
            label="X Gravity",
        )
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.gravity[1]"],
            label="Y Gravity",
        )
        plt.plot(
            df[time_col],
            df["sesmt.md.merged_frame.gravity[2]"],
            label="Z Gravity",
        )
...
def compute_average_acceleration(df, start_time=40, end_time=90):
    ...
    try:
        ...
        time_col = (
            "sys.exec.out.time"
            if "sys.exec.out.time" in df.columns
        ...
        if filtered_df.empty:
            print(
                f"No data found within the time range {start_time}s to {end_time}s"
            )
            return None
