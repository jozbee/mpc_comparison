...
def extract_dynamics_from_sms(sms_df, resample_dt=None, start_time=40.0):
    ...
    time_col = 'sys.exec.out.time' if 'sys.exec.out.time' in sms_df.columns else 'sesmt.md.merged_frame.time'
...
def generate_stewart_path_from_sms(
    ...
    print(f"Simulating {num_steps} steps over {timestamps[-1] - timestamps[0]:.2f} seconds")
    ...
    for i in range(num_steps):
        ...
        # Print progress
        if i % 100 == 0:
            print(f"Processed step {i}/{num_steps} ({i/num_steps*100:.1f}%)")
...
def plot_sms_vs_mpc_dynamics(
    ...
    ax6.plot(waypoints_array[:, 0], waypoints_array[:, 1], waypoints_array[:, 2])
...
if __name__ == "__main__":
    ...
    # Resample to a reasonable step size for MPC
    resample_dt = 0.05  # 50ms between samples
    timestamps, sms_linear_acc, sms_angular_vel = extract_dynamics_from_sms(sms_df, resample_dt)
    assert timestamps is not None, "Failed to extract timestamps from SMS data"
    assert sms_linear_acc is not None, "Failed to extract linear accelerations from SMS data"
    assert sms_angular_vel is not None, "Failed to extract angular velocities from SMS data"
    ...
    waypoints, mpc_linear_acc, mpc_angular_vel = generate_stewart_path_from_sms(
        timestamps, sms_linear_acc, sms_angular_vel, max_duration,
    )
    ...
    anim, fig_viz = visualize_trajectory(waypoints, dt=resample_dt, sim_rate=2.0, fps=30)
