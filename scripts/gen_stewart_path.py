...
def generate_constant_acceleration_path(acceleration=0.05, duration=5.0, dt_sim=0.1):
    """
    Generate a path with constant acceleration in the x direction using the MPC controller.
    
    Args:
        acceleration: Desired acceleration in x direction (m/s²)
        duration: Total duration of the path (seconds)
        dt_sim: Time step for simulation (seconds)
        
    Returns:
        Tuple containing:
        - List of waypoints, each a [x, y, z, roll, pitch, yaw] pose
        - List of linear accelerations [x_ddot, y_ddot, z_ddot]
        - List of angular velocities [omega_x, omega_y, omega_z]
    """
    ...
    for i in range(num_steps):
        ...
        # First row: omega_x = phi_dot - sin(theta) * psi_dot
        omega_x = phi_dot - np.sin(theta) * psi_dot
        # Second row: omega_y = cos(phi) * theta_dot + sin(phi) * cos(theta) * psi_dot
        omega_y = np.cos(phi) * theta_dot + np.sin(phi) * np.cos(theta) * psi_dot
        # Third row: omega_z = -sin(phi) * theta_dot + cos(phi) * cos(theta) * psi_dot
        omega_z = -np.sin(phi) * theta_dot + np.cos(phi) * np.cos(theta) * psi_dot
    ...
    return waypoints, np.array(linear_accelerations), np.array(angular_velocities)
...
if __name__ == "__main__":
    ...
    waypoints, linear_accelerations, angular_velocities = generate_constant_acceleration_path(
